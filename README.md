...
- **OAuth Upstreams**: Authorize Codex, Claude, and Gemini accounts from the Web UI. They participate in the same ordering, pinning, enable/disable, and failover flow as API-key providers, and Clipal refreshes access tokens automatically when a refresh token is available. Claude and Codex OAuth requests use Agent SDK-compatible wrapping by default for ordinary clients, with required transport fields handled by Clipal. Explicit request fields such as tools, thinking/reasoning, and output controls take precedence over Clipal defaults when supported by the target model.
