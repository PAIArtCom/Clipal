...
func TestLiveClaudeOAuthSmokeScriptUsesTemporaryCredentialCopyAndRefreshProbe(t *testing.T) {
	...
	if !strings.Contains(billing, "cc_version=2.1.220.5bd; cc_entrypoint=sdk-cli; cch=00000;") {
	...
	if !strings.Contains(countTokensBilling, "cc_version=2.1.220.5bd; cc_entrypoint=sdk-cli; cch=00000;") {
	...
	if !strings.Contains(emojiBilling, "cc_version=2.1.220.") {
...
func runClaudeSmokePayloadPython(t *testing.T, py string, script string, anchor string, prompt string) map[string]any {
	...
	args = append(args, "2.1.220", "59cf53e54c78")
