...
| `oauth_provider` | string | OAuth only | Supported combinations are `codex` in `openai.yaml`, `claude` in `claude.yaml`, and `antigravity` in `gemini.yaml` |
...
- Supported today: `openai.yaml` with `auth_type: oauth` and `oauth_provider: codex`; `claude.yaml` with `auth_type: oauth` and `oauth_provider: claude`; `gemini.yaml` with `auth_type: oauth` and `oauth_provider: antigravity`. Legacy `oauth_provider: gemini` configs still load for compatibility, but new Gemini CLI OAuth authorizations are no longer offered.
- Current protocol scope: Codex OAuth supports OpenAI `/v1/responses*`; Claude OAuth supports `/v1/messages` and `/v1/messages/count_tokens`; Antigravity OAuth supports Gemini-compatible `generateContent`, `streamGenerateContent`, `countTokens`, model listing, and Gemini image models through `generateContent`. It does not wrap Imagen/Veo `predict*` endpoints; use Google AI Studio API-key or Vertex providers for those routes.
- Claude OAuth requests use a lightweight Agent SDK-compatible envelope by default, with current transport headers and billing fingerprints generated by Clipal.
- Codex OAuth Responses requests are normalized into the lightweight Agent SDK-compatible shape, with required transport fields handled by Clipal.
- The bundled OAuth transport identities currently track Codex CLI 0.145.0, Claude Code 2.1.220, and Antigravity 2.0 2.4.2. On macOS, the Antigravity identity uses the installed application's version when available; `CLIPAL_OAUTH_ANTIGRAVITY_VERSION` can override it for a controlled deployment.
- Defaults are non-destructive when the target model supports the field. If the client already sends `tools`, Claude `thinking` / `context_management` / `output_config`, or Codex `reasoning` / `tool_choice` / `parallel_tool_calls`, Clipal preserves those fields instead of overwriting them; unsupported Claude models may still drop incompatible thinking/output controls.
- Do not set `base_url`, `api_key`, or `api_keys` on an OAuth provider
- Create OAuth providers from the Web UI by choosing `Add Provider -> OAuth -> Codex`, `Claude`, or `Antigravity` on the matching client page
...
- Use `https://generativelanguage.googleapis.com` for Google AI Studio / Gemini API keys. Clipal forwards Gemini `generateContent`, streaming, token counting, embeddings, Imagen/Veo `predict*`, files, cached contents, tuned models, `interactions`, batches, operations, file search stores, generated files, corpora, auth tokens, agents, and webhooks without changing the request body. Gemini Live API WebSocket proxying is not supported yet.
- Use a regional Vertex AI base URL such as `https://us-central1-aiplatform.googleapis.com` for Vertex Gemini REST requests. Put the short-lived OAuth/service-account access token in `api_key` or `api_keys`; Clipal sends it as `Authorization: Bearer ...` for Vertex hosts.
