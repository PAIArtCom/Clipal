...
Preview never writes data. If the selected data or current state changes after preview, Clipal rejects Apply and requires a new review. Apply snapshots configuration, credentials, and usage first; if persistence or runtime reload fails, Clipal restores all three snapshots.
