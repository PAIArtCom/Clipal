...
Open **Global Settings**. In the separate **Export Full Backup** panel, select the destination and download `clipal-data.json`; it includes global and client configuration, provider API keys, OAuth access and refresh tokens, and usage data. In the separate **Import Data** panel, select one native backup or one or more external credential JSON files, then use **Preview Import** to open an import-review dialog showing the detected source, mode, planned changes, and warnings. Apply only from that reviewed dialog.

Preview never writes data. Clipal rejects Apply and requires a new review if the selected data, configuration, or credential account set changes after preview. Routine usage recording and token refreshes for the same accounts do not invalidate a reviewed plan. Apply snapshots configuration, credentials, and usage first; if persistence or runtime reload fails, Clipal restores all three snapshots.
