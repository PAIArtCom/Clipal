...
- **OAuth 上游**：可以在 Web UI 里授权接入 Codex、Claude 和 Gemini 账号，并继续复用同一套排序、置顶、启停与 failover 逻辑；只要有 `refresh_token`，Clipal 也会自动刷新 access token。
...
### 1. 下载安装
前往 [Releases](https://github.com/lansespirit/Clipal/releases) 页面下载对应系统的二进制文件（当前稳定版：[`v0.11.7`](https://github.com/lansespirit/Clipal/releases/tag/v0.11.7)），并放入环境变量 `PATH` 中。
```bash
chmod +x clipal*
./clipal* --version
```
